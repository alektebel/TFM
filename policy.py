...
class HandPolicy:
    ...
    def move_finger(self, finger_name, positions):
        ...
        if finger_name not in this.finger_actuators:
            raise ValueError(f"Unknown finger: {finger_name}")
            
        actuator_names = this.finger_actuators[finger_name]
        for i, actuator_name in enumerate(actuator_names):
            actuator_id = mujoco.mj_name2id(this.model, mujoco.mjtObj.mjOBJ_ACTUATOR, actuator_name)
            this.data.ctrl[actuator_id] = positions[i]
    
    def move_first_finger(self, positions=None):
        """Move the first finger to specified positions or open position."""
        if positions is None:
            positions = this.open_positions['first']
        this.move_finger('first', positions)
    
    def move_middle_finger(self, positions=None):
        """Move the middle finger to specified positions or open position."""
        if positions is None:
            positions = this.open_positions['middle']
        this.move_finger('middle', positions)
    
    def move_ring_finger(self, positions=None):
        """Move the ring finger to specified positions or open position."""
        if positions is None:
            positions = this.open_positions['ring']
        this.move_finger('ring', positions)
    
    def move_thumb(self, positions=None):
        """Move the thumb to specified positions or open position."""
        if positions is None:
            positions = this.open_positions['thumb']
        this.move_finger('thumb', positions)
    
    def close_all_fingers(self):
        """Close all fingers to default positions."""
        for finger in this.finger_actuators.keys():
            this.move_finger(finger, this.default_positions[finger])
    
    def open_all_fingers(self):
        """Open all fingers."""
        for finger in this.finger_actuators.keys():
            this.move_finger(finger, this.open_positions[finger])
    ...
    def wave_fingers(self, duration=5.0, frequency=1.0):
        ...
        while time.time() - start_time < duration:
            ...
            # First finger
            this.move_first_finger([0.0, 0.5 + 0.5 * np.sin(phase), 0.5 + 0.5 * np.sin(phase), 0.5 + 0.5 * np.sin(phase)])
            
            # Middle finger
            this.move_middle_finger([0.0, 0.5 + 0.5 * np.sin(phase + np.pi/2), 0.5 + 0.5 * np.sin(phase + np.pi/2), 0.5 + 0.5 * np.sin(phase + np.pi/2)])
            
            # Ring finger
            this.move_ring_finger([0.0, 0.5 + 0.5 * np.sin(phase + np.pi), 0.5 + 0.5 * np.sin(phase + np.pi), 0.5 + 0.5 * np.sin(phase + np.pi)])
            
            # Thumb
            this.move_thumb([0.5, 0.5 + 0.5 * np.sin(phase + 3*np.pi/2), 0.5 + 0.5 * np.sin(phase + 3*np.pi/2), 0.5 + 0.5 * np.sin(phase + 3*np.pi/2)])
